...
IF (!function_exists("obc_cache_init")){
	FUNCTION obc_cache_init(){
		...
		IF (!$obc_configuration["plugin_active"] || !$obc_configuration["site_url"]) RETURN false;
		define("OBC_SITE_URL",obc_clean_url($obc_configuration["site_url"]));
		define("OBC_GZIP",$obc_configuration["gzip_enabled"]);
		define("OBC_COOKIEHASH",md5($obc_configuration["site_url"]));
		...
		obc_cache("http://".$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]);
	};
};
